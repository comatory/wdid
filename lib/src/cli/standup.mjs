...
import { askModifications, main, success } from './utils.mjs'
import { formatEntry, printEntryDescriptor } from './log.mjs'
import {
  standupQuestion1,
  standupQuestion2,
  standupQuestion3,
...
export const processNewCommand = async (args, dbService) => {
  ...
  if (existingStandupEntry) {
    ...
    if (previousReminderEntry) {
      ...
      printEntryDescriptor(formatEntry(previousReminderEntry))
      console.log('\n')
    }
