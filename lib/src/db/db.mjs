 ...
 * @property {(Date) => Promise<?Entry>} getPreviousWorkingDayStandupReminder - get stand-up reminder entry based on stand-up entry creation date
 ...
 * stand-up entry
 * @param {Date} createdAt - stand-up entry creation date time
 * @returns {Promise<?Entry>} entry
 */
export const getPreviousWorkingDayStandupReminder = async (createdAt) => {
  const db = await get()
  ...
  const previousDay = new Date(copy - 1)
...
INNER JOIN entries ON entries.id = answers.entry_id
INNER JOIN entry_types ON entry_types.id = entries.type_id
WHERE entries.type_id = $type
AND entries.created_at BETWEEN $today_start AND $today_end;
`
